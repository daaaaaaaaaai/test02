...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('material_document_items', function (Blueprint $table) {
            ...
            $table->char('item_number',5)->comment('入出庫伝票明細');
            ...
            $table->string('text_item',255)->comment('明細テキスト')->nullable();
            $table->text('remarks_item')->comment('明細備考')->nullable();
            $table->string('created_by',10)->comment('登録ユーザID');
            $table->string('changed_by',10)->comment('変更ユーザID');
            $table->timestamps();
            $table->softDeletes();

            $table->comment('入出庫伝票明細');
            $table->primary(['mat_doc_no','item_number']);
            $table->unique(['mat_doc_no','item_number']);
        });
    }
